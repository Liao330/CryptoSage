...
你的任务是分析衍生品市场数据（资金费率、OI、爆仓、期权交割），输出结构化的衍生品信号。
...
5. 期权交割：关注最近到期时间、Call/Put OI、最大痛点、Call/Put 墙。
   ...
   - 期权墙与最大痛点和现货/永续信号同向时可作为弱确认，冲突时必须标注“可能收割大头仓位”，降低置信度。
...
async def run_derivatives_agent(state: AnalysisState) -> AnalysisState:
    ...
    user_prompt = (
        ...
        "爆仓数据 get_liquidations、Deribit 期权交割与最大痛点 get_options_snapshot），综合分析后输出 JSON 格式的衍生品分析信号。"
    ...
    async_tool_map = {
        ...
        "get_options_snapshot": derivatives_client.get_options_snapshot,
    }

    reasoning = ""
    tool_calls_log: list = []
    try:
        fc = await run_function_calling(SYSTEM_PROMPT, user_prompt, DERIVATIVES_TOOLS, async_tool_map)
        ...
        funding = await derivatives_client.get_funding_rate(bin_symbol)
        oi = await derivatives_client.get_open_interest(bin_symbol)
        liquidations = await derivatives_client.get_liquidations(bin_symbol)
    ...
    # 避免衍生品信号长期只有永续合约数据。
    options_snapshot = await derivatives_client.get_options_snapshot(symbol_raw)
    ...
    trace.append({
        ...
        "reasoning": reasoning,
        "tool_calls": [t.get("function") for t in tool_calls_log] + ["get_options_snapshot"],
    })
    state["trace"] = trace
